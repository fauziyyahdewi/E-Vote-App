        ...
        <div className="relative transform overflow-hidden bg-white text-left shadow-xl transition-all p-4 rounded-md">
          ...
          <div className="w-full p-5 text-center">
            ...
            <div className={`space-x-3 mt-5`}>
              <button className="text-sm bg-orange-100 px-2 py-1 hover:bg-zinc-200" onClick={() => { props.onNegativeClick; setIsOpen(false); }}>{props.negativeText || "Kembali"}</button>
              <button className={`${!props.onPositiveClick && "hidden"} text-sm bg-black px-2 py-1 text-white hover:bg-zinc-200 hover:text-black`} onClick={()=>{ props.onPositiveClick && props.onPositiveClick(); setIsOpen(false) } }>{props.positiveText || "Ya"} </button>
            </div>
          </div>
